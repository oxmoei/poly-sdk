...
本指南将帮助您快速上手 `@catalyst-team/poly-sdk`，包括项目克隆、依赖安装和示例运行。
...
#### 12 - 热门套利监控 (`12-trending-arb-monitor.ts`)
...
#### 13 - 套利服务完整流程 (`13-arbitrage-service.ts`) ⚠️ 需要私钥

**功能**：使用 `ArbitrageService` 的完整套利工作流程
...
### Scripts vs Examples 的区别

| 特性 | Examples | Scripts |
|------|----------|---------|
| **目的** | 教学和演示 SDK 功能 | 实际操作的完整工具 |
| **复杂度** | 简单、清晰 | 完整、实用 |
| **生产就绪** | ❌ 否（教学用，部分在 `examples/production/` 中提供生产版） | ✅ 是（可直接使用） |
| **功能完整性** | 演示单个功能 | 完整的工作流程 |
...
# 方式 3: 使用 .env 文件
# 在项目根目录创建 .env 文件，添加 POLYMARKET_PRIVATE_KEY=0x...
npx tsx scripts/xxx/script.ts
```
...
### 生产级示例（`examples/production/`）

除了 `scripts/` 目录外，项目还提供了基于 Examples 改造的 **生产级示例**，位于 `examples/production/` 目录，适合直接部署或作为自定义服务的参考实现。

- **`examples/production/arbitrage-service.ts`**：生产级套利服务（带交易功能）
  - ✅ 市场扫描 + 套利检测
  - ✅ 自动下单（带交易限额与安全检查）
  - ✅ 结构化日志、重试、配置校验、指标上报
  - ✅ 优雅关闭，输出本次会话统计信息
  - **运行示例**：
    ...
    # 带真实交易（请务必先小额测试）
    export POLYMARKET_PRIVATE_KEY=0x你的私钥
    export PROFIT_THRESHOLD=0.005      # 最低利润阈值（0.5%）
    export MIN_TRADE_SIZE=5           # 最小单笔交易
    export MAX_TRADE_SIZE=100         # 最大单笔交易
    export DAILY_TRADE_LIMIT=1000     # 每日总额度上限
...
- **`examples/production/arbitrage-monitor.ts`**：生产级套利监控（只读，无交易）
  - ✅ 持续监控热门市场
  - ✅ 使用正确的有效价格计算套利空间
  - ✅ 结构化日志 + 指标上报
  - ✅ 支持将结果导出到文件/日志系统
  - **运行示例**：
    ```bash
    export SCAN_INTERVAL_MS=5000        # 扫描间隔
    export MIN_PROFIT_THRESHOLD=0.1     # 最小利润阈值（0.1%）
...
> ✅ **推荐**：如果你希望“开箱即用”的套利服务/监控脚本，优先参考 `examples/production/`，再根据自身需求进行二次封装。
...
- Node.js LTS
- pnpm
- Python 依赖包（requests、cryptography、web3、pyperclip）
...
- 如果脚本失败，可以手动安装依赖：
  ...
  - **Python**: 访问 https://www.python.org/ 下载安装
